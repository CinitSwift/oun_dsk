...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/pnpm-%3E%3D10-blue" alt="pnpm >=10"/>
...
- [🚀 部署](#-部署)
  ...
  - [本地运行](#本地运行)
- [🖥️ 桌面应用（Electron）](#️-桌面应用electron)
- [🔄 更新同步](#-更新同步)
  - [Vercel 更新](#vercel-更新)
  - [Docker 更新](#docker-更新)
  - [本地更新](#本地更新)
...
桌面版基于 Electron 打包，应用内置前端页面和本地回环代理。用户无需另装 Node.js、pnpm 或 Docker。
...
该命令会同时启动 Vite 开发服务器和 Electron，代理由 Electron 进程管理并通过前端地址注入。开发服务器固定使用 `3000` 端口；如果 `3000` 已被占用，Vite 的 `strictPort` 配置会使开发服务明确失败，而不会自动切换端口。
...
在 Windows 实机或 Windows CI 上构建 NSIS 安装程序：

```bash
pnpm desktop:build:win
```

构建产物为 `release/` 下的 NSIS `.exe`。当前 macOS 环境不能证明 Windows 实机上的安装和运行行为，因此 Windows 安装行为需要在 Windows 实机或 CI 中单独验证；不要将 macOS 构建结果视为 Windows 验证。

### 运行说明与当前限制

- 应用内置前端和本地回环代理，用户无需另装 Node.js、pnpm 或 Docker。
- 视频搜索、视频源 API 以及 HLS/MP4 媒体播放仍需联网。
- 关闭窗口时应用会隐藏到系统托盘；通过托盘可以恢复主窗口或退出应用。
- 第一版不包含自动更新、开机启动、Linux 支持、离线缓存、原生下载器，以及代码签名/公证。
